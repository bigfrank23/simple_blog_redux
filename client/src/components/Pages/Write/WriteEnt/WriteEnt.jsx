...
import {createPost} from '../../../../redux/actions/entPosts'


import '../Write.css'
import './WriteEnt.css'

  // const initialState = {username: "", profilePic: "", title: "", desc: "", label: "Entertainment"}
...
const WriteEnt = () => {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      // dispatch(createPost({ ...writeData, username: user.user.username }));
        dispatch(createPost({ ...writeData, username: user.user.username, profilePic: user.user.profilePic, label: "Entertainment" }))
        setShowText(true);
        setTimeout(() => {
          setShowText(false);
        }, 5000);
  ...
  return (
    <div className="container" id='ent_container' >
      <div className="row shadow">
          <div className="text text-center text-secondary my-2">
              <p><i>Write About Entertainment</i></p>
          </div>
        <div className="d-flex justify-content-center">
          <img src={!writeData.selectedFile ? WriteImg : writeData.selectedFile} className='ent_img' alt="WImg" />
        </div>
