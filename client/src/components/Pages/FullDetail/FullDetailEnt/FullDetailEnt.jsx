...
import { getPosts, getSinglePost } from '../../../../redux/actions/entPosts'

import '../../../Detail/Detail.css'
import '../FullDetailSports/FullDetailSports.css'

import CommentSectionEnt from '../../CommentSection/CommentSectionEnt/CommentSectionEnt'
import axios from 'axios'
import { LIKE } from '../../../../redux/constants/actionTypes'
import SideBar from '../../../SideBar/SideBar'

const FullDetailEnt = (props) => {
    ...
    const handleLikes = async (userId, postId) => {
        if (!post.likes.includes(userId)) {
            try {
                const res = await axios.put("http://localhost:5000/server/ent/likePost", {userId, postId})
            ...
            try {
                const res = await axios.put("http://localhost:5000/server/ent/unLikePost", {userId, postId})
                dispatch({type: LIKE, payload: res.data}) 
                // console.log(res.data);
            } catch (error) {
                console.log(error);
  ...
  return (
    <div className='container-fluid' id='detail_container' style={{marginTop: "9rem"}}>
        <div className="row d-flex d-flex flex-row-reverse justify-content-between">
        <SideBar />
            <div className="col-lg-7 col-md-12 col-sm-12 p-4 p-md-3 p-lg-3 bg-light shadow">
                ...
                <div className="detail_center">
                    <div className="detail_img_box">
                        <img className='detail_img2 rounded' src={post.selectedFile} alt="Dt2Img" />
                    </div>
